...
def analyze_Semi_data(df):
    ...
    if 'BatadcPC' in df.columns and not df['BatadcPC'].isna().all():
        jig_column = 'BatadcPC'
    elif 'SemiAssyMaxSolarVolt' in df.columns and not df['SemiAssyMaxSolarVolt'].isna().all():
        jig_column = 'SemiAssyMaxSolarVolt'
    else:
        # JIG 정보가 없는 경우 기본값으로 "DEFAULT_JIG" 사용
        df['DEFAULT_JIG'] = 'SemiAssy_JIG'
        jig_column = 'DEFAULT_JIG'
    
    summary_data = {}
    
    for jig, group in df.groupby(jig_column):
